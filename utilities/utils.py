...
def matching_bert_emb_id(user, item, rating, user_embeddings, item_embeddings):
    ...
    while i < dim_X_rows:
        ...
        user_embedding = (
            np.array(
                user_embeddings.loc[
                    (user_embeddings["ID_OpenKE"] == user_id), "profile_embedding"
                ]
            )
        )[0]
        item_embedding = (
            np.array(
                item_embeddings.loc[
                    (item_embeddings["ID_OpenKE"] == item_id), "embedding"
                ]
            )
        )[0]
...
def matching_Bert_Graph_conf(
    ...
    while i < dim_X_rows:
        ...
        user_bert_embedding = (
            np.array(
                user_bert_embeddings.loc[
                    (user_bert_embeddings["ID_OpenKE"] == user_id), "profile_embedding"
                ]
            )
        )[0]
        item_bert_embedding = (
            np.array(
                item_bert_embeddings.loc[
                    (item_bert_embeddings["ID_OpenKE"] == item_id), "embedding"
                ]
            )
        )[0]
