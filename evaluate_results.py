...
if __name__ == "__main__":
    ...
    try:
        if not os.path.isdir(predictions_path):
            raise RuntimeError(
                "Invalid predictions path specified. Unable to run evaluator."
            )
            ...
            if num_filtered_files == 1:
                ...
                print(
                    [
                        "java",
                        "-jar",
                        "experiments/binaries/mimir.jar",
                        "-holdout",
                        "-cutoff",
                        cutoff,
                        "-test",
                        test_filename,
                        "-predictions",
                        os.sep.join([root, files[0]]),
                        "-results",
                        results_filename,
                    ]
                )
                mimir_output = subprocess.check_output(
                    [
                        "java",
                        "-jar",
                        "experiments/binaries/mimir.jar",
                ...
                mimir_output = subprocess.call(
                    [
                        ...
                        test_filename,
                        "-predictions",
                        os.sep.join([root, "predictions_%s.tsv"]),
                        "-results",
                        results_filename,
                    ]
                )
